...
VALUES (9999,'홍길동','PRESIDENT',NULL,'2000/01/01',5000,1000,10);

SELECT * FROM EMP_TEMP;
-- INSERT 문으로 날짜 데이터 입력하기 (날짜 사이에 / 입력하기)
...
-- 날짜 데이터 형식을 반대로 했을때 오류뜸.

INSERT INTO EMP_TEMP(EMPNO,ENAME,JOB,MGR,HIREDATE,SAL,COMM,DEPTNO) 
VALUES (2111,'이순신','MANAGER',9999,TO_DATE('07/01/2001','DD/MM/YYYY'),4000,NULL,20);

SELECT * FROM EMP_TEMP;
-- TO_DATE 함수를 사용하여 날짜 데이터 입력하기 TO_DATE 를 사용하여 순서를 바꿔서 가능
...
FROM DEPT_TEMP2 WHERE DNAME = 'OPERATIONS');
...
-- EMP테이블을 복사해서 EMP_TMEP2 테이블만들기

DELETE FROM EMP_TEMP2 WHERE JOB = 'MANAGER';

SELECT * FROM EMP_TEMP2;
-- WHERE 사용해서 데이터 일부 삭제하기
...
-- SCOTT 계정에서 사용 가능한 데이터 사전 살펴보기 (DICTIONARY사용)
...
-- SCOTT 계정으로 사용자 생성하기 (오류 뜸:SCOTT 계정은 권한X)
